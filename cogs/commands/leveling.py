                ...
                async with aiosqlite .connect ("db/leveling.db")as db :
                    ...
                    if exists :
                        ...
                        await db .execute ("""
                            ...
                            SET enabled = ?, xp_per_message = ?, min_xp = ?, max_xp = ?, level_message = ?, embed_color = ?, 
...
class Leveling (commands .Cog ):
        ...
        try :
            async with aiosqlite .connect ("db/leveling.db")as db :
                ...
                async with db .cursor ()as cursor :
                    ...
                    await cursor .execute ("PRAGMA table_info(leveling_settings)")
                    setting_columns =[col [1 ]for col in await cursor .fetchall ()]
                    if "rank_background"not in setting_columns :
                        await cursor .execute ("ALTER TABLE leveling_settings ADD COLUMN rank_background TEXT")
                        logger .info ("Added rank_background column to leveling_settings table")
            ...
            try :
                async with aiosqlite .connect ("db/leveling.db")as db :
                    ...
                    if not row :
                        ...
                        await db .execute ("INSERT OR IGNORE INTO leveling_settings (guild_id, enabled) VALUES (?, 0)",(guild_id ,))
                        await db .commit ()
                        return {
                        ...
                        'dm_level_up':False ,'rank_background':None 
                        }
                    ...
                    'rank_background':row [12 ]if len (row )>12 else None 
                    }
    ...
    async def on_message (self ,message ):
        ...
        try :
            ...
            if min_xp is None or max_xp is None :
                ...
                base_xp =random .randint (min_xp ,max_xp )if max_xp >min_xp else min_xp 
    ...
    async def handle_level_up (self ,message ,new_level ,settings ):
        ...
        try :
            ...
            color =embed_color if isinstance (embed_color ,int )and embed_color >0 else 0xFF0000 ,
            timestamp =datetime .now (timezone .utc )
            )
...
└─────────────────────────────────────────┘
    ...
    async def fetch_image_bytes (self ,url :str ,timeout :int =10 )->bytes :
    ...
    async def get_avatar_image (self ,member ,size :int ):
        """Fetch a member's profile picture as a square RGBA image"""
        avatar =member .display_avatar .replace (size =256 ,static_format ="png")
        data =await self .fetch_image_bytes (str (avatar .url ))
        return Image .open (io .BytesIO (data )).convert ("RGBA").resize ((size ,size ))

    def paste_circular_avatar (self ,base ,avatar_img ,x :int ,y :int ,ring =(255 ,255 ,255 )):
        """Paste a profile picture as a circle with a ring onto the base image"""
        size =avatar_img .size [0 ]
        mask =Image .new ('L',(size ,size ),0 )
        ImageDraw .Draw (mask ).ellipse ((0 ,0 ,size -1 ,size -1 ),fill =255 )
        ImageDraw .Draw (base ).ellipse ((x -4 ,y -4 ,x +size +4 ,y +size +4 ),fill =ring )
        base .paste (avatar_img ,(x ,y ),mask )

    async def set_rank_background (self ,guild_id :int ,url ):
        """Store (or clear) the custom rank-card background for a guild"""
        async with aiosqlite .connect (self .db_path )as db :
            await db .execute ("INSERT OR IGNORE INTO leveling_settings (guild_id, enabled) VALUES (?, 0)",(guild_id ,))
            await db .execute ("UPDATE leveling_settings SET rank_background = ? WHERE guild_id = ?",(url ,guild_id ))
            await db .commit ()

    async def create_custom_background_design (self ,member ,xp ,level ,rank ,messages ,current_level ,progress ,needed ,width ,height ,font_large ,font_medium ,font_small ,background_url ):
        """Rank card rendered on top of the server's custom background image"""
        data =await self .fetch_image_bytes (background_url )
        background =Image .open (io .BytesIO (data )).convert ("RGB")

        scale =max (width /background .width ,height /background .height )
        background =background .resize ((max (width ,int (background .width *scale )),max (height ,int (background .height *scale ))))
        left =(background .width -width )//2 
        top =(background .height -height )//2 
        img =background .crop ((left ,top ,left +width ,top +height )).convert ("RGBA")

        overlay =Image .new ("RGBA",(width ,height ),(0 ,0 ,0 ,130 ))
        img =Image .alpha_composite (img ,overlay )
        draw =ImageDraw .Draw (img )

        draw .rounded_rectangle ((15 ,15 ,width -15 ,height -15 ),radius =20 ,outline =(255 ,255 ,255 ),width =2 )

        def shadow_text (pos ,text ,font ,fill ):
        ...
        try :
            avatar_img =await self .get_avatar_image (member ,avatar_size )
            self .paste_circular_avatar (img ,avatar_img ,avatar_x ,avatar_y )
        except Exception as e :
            logger .error (f"Avatar fetch failed on custom background: {e}")
            draw .ellipse ((avatar_x ,avatar_y ,avatar_x +avatar_size ,avatar_y +avatar_size ),fill =(64 ,68 ,75 ))


        info_x =avatar_x +avatar_size +30 
        stats_x =690 
        info_max_width =stats_x -40 -info_x 


        username =member .display_name 
        while username and text_size (username ,font_large )[0 ]>info_max_width :
            username =username [:-1 ]
        shadow_text ((info_x ,58 ),username ,font_large ,(255 ,255 ,255 ))


        level_text =f"🏆 Level {level}"
        shadow_text ((info_x ,105 ),level_text ,font_medium ,(255 ,215 ,0 ))
        rank_x =info_x +text_size (level_text ,font_medium )[0 ]+30 
        shadow_text ((rank_x ,105 ),f"📊 Rank #{rank}",font_medium ,(135 ,206 ,250 ))


        bar_x ,bar_y =info_x ,165 
        bar_width ,bar_height =info_max_width ,26 
        draw .rounded_rectangle ((bar_x ,bar_y ,bar_x +bar_width ,bar_y +bar_height ),radius =13 ,fill =(20 ,20 ,24 ))
        draw .rounded_rectangle ((bar_x ,bar_y ,bar_x +bar_width ,bar_y +bar_height ),radius =13 ,outline =(200 ,200 ,200 ),width =1 )
        if needed >0 and progress >0 :
            fill_width =max (6 ,int ((min (progress ,needed )/needed )*bar_width ))
            draw .rounded_rectangle ((bar_x ,bar_y +2 ,bar_x +fill_width ,bar_y +bar_height -2 ),radius =11 ,fill =(114 ,137 ,218 ))

        xp_text =f"{progress:,} / {needed:,} XP"
        xp_w ,xp_h =text_size (xp_text ,font_small )
        shadow_text ((bar_x +(bar_width -xp_w )//2 ,bar_y +(bar_height -xp_h )//2 -2 ),xp_text ,font_small ,(255 ,255 ,255 ))


        percentage =(progress /needed *100 )if needed >0 else 0 
        shadow_text ((info_x ,bar_y +bar_height +12 ),f"{percentage:.1f}% to level {level+1}",font_small ,(220 ,220 ,220 ))


        stat_rows =[
        ("💬 Messages",f"{messages:,}",(0 ,255 ,127 )),
        ("⭐ Total XP",f"{xp:,}",(255 ,165 ,0 )),
        ]
        stat_y =66 
        for label ,value ,color in stat_rows :
            shadow_text ((stats_x ,stat_y ),label ,font_small ,(235 ,235 ,235 ))
            shadow_text ((stats_x ,stat_y +24 ),value ,font_medium ,color )
            stat_y +=70 
    ...
    async def create_rank_card_design (self ,member ,xp ,level ,rank ,messages ,design_number ):
        ...
        try :
            guild_settings =await self .get_guild_settings (member .guild .id )
            background_url =guild_settings .get ('rank_background')
        except Exception as e :
            logger .error (f"Error loading rank background: {e}")
            background_url =None 

        if background_url :
            try :
                return await self .create_custom_background_design (member ,xp ,level ,rank ,messages ,current_level ,progress ,needed ,width ,height ,font_large ,font_medium ,font_small ,background_url )
            except Exception as e :
                logger .error (f"Custom background failed, using built-in design: {e}")
    ...
    async def rank (self ,ctx ,member :Optional [discord .Member ]=None ):
        ...
        try :
            ...
            if rank_card :
                ...
                await ctx .send (f"{CROSS} Couldn't build a rank card right now. Please try again.")
        except Exception as e :
            logger .error (f"Error in rank command: {e}")
            await ctx .send (f"{CROSS} Something went wrong while building the rank card.")
        ...
        try :
            ...
            async with aiosqlite .connect ("db/leveling.db")as db :
                ...
                if exists :
                    ...
                    await db .execute ("""
                        UPDATE leveling_settings 
                        SET enabled = 1, xp_per_message = ?, min_xp = ?, max_xp = ?, level_message = ?, embed_color = ?, 
                            level_image = ?, thumbnail_enabled = ?
                        WHERE guild_id = ?
                    """,(
                    xp_per_message ,
    ...
    @level .command (name ="setxp",description ="Set a fixed XP amount per message")
    @commands .has_permissions (administrator =True )
    async def setxp (self ,ctx ,amount :int ):
        """Set a fixed XP amount per message (same as xprange with a single value)"""
        try :
            ...
            async with aiosqlite .connect (self .db_path )as db :
                ...
                was_disabled =not row or not row [0 ]

                if row :
                    await db .execute ("UPDATE leveling_settings SET xp_per_message = ?, min_xp = ?, max_xp = ?, enabled = 1 WHERE guild_id = ?",(amount ,amount ,amount ,ctx .guild .id ))
                else :
                    await db .execute ("INSERT INTO leveling_settings (guild_id, enabled, xp_per_message, min_xp, max_xp) VALUES (?, 1, ?, ?, ?)",(ctx .guild .id ,amount ,amount ,amount ))

                await db .commit ()

            description =f"Members will now get **{amount} XP** per message."
            if was_disabled :
                description +="\n\n⚡ Leveling was **disabled** — it has been enabled automatically."
    ...
    @level .command (name ="xprange",description ="Set the XP range per message",aliases =['range','xpr'])
    @commands .has_permissions (administrator =True )
    async def xprange (self ,ctx ,min_xp :int ,max_xp :Optional [int ]=None ):
        """Set the XP range per message — one value means a fixed amount"""
        try :
            if max_xp is None :
                max_xp =min_xp 

            if min_xp <1 or max_xp >999 :
                await ctx .send (f"{CROSS} XP values must be between 1 and 999!")
                return 
            ...
            async with aiosqlite .connect (self .db_path )as db :
                ...
                if row :
                    await db .execute ("UPDATE leveling_settings SET min_xp = ?, max_xp = ?, xp_per_message = ?, enabled = 1 WHERE guild_id = ?",(min_xp ,max_xp ,min_xp ,ctx .guild .id ))
                else :
                    await db .execute ("INSERT INTO leveling_settings (guild_id, enabled, xp_per_message, min_xp, max_xp) VALUES (?, 1, ?, ?, ?)",(ctx .guild .id ,min_xp ,min_xp ,max_xp ))

                await db .commit ()

            if min_xp ==max_xp :
                description =f"Members will now get **{min_xp} XP** per message."
            else :
                description =f"Members will now get a random amount between **{min_xp}** and **{max_xp} XP** per message."

            if was_disabled :
                description +="\n\n⚡ Leveling was **disabled** — it has been enabled automatically."

            embed =discord .Embed (
            title =f"{TICK} XP Range Updated",
            description =description ,
            color =0x00FF00 ,
            timestamp =datetime .now (timezone .utc )
            )
    ...
    async def setmessage (self ,ctx ,*,message :str ):
        ...
        try :
            ...
            async with aiosqlite .connect (self .db_path )as db :
                ...
                if exists :
                    await db .execute ("UPDATE leveling_settings SET level_message = ?, enabled = 1 WHERE guild_id = ?",(message ,ctx .guild .id ))
                else :
                    await db .execute ("INSERT INTO leveling_settings (guild_id, enabled, level_message) VALUES (?, 1, ?)",(ctx .guild .id ,message ))
    ...
    @level .command (name ="setcolor",description ="Set embed color",aliases =['color','colour','embedcolor'])
    @commands .has_permissions (administrator =True )
    async def setcolor (self ,ctx ,color :str ):
        """Set the embed color used for level-up messages"""
        try :
            ...
            async with aiosqlite .connect (self .db_path )as db :
                ...
                if row :
                    await db .execute ("UPDATE leveling_settings SET embed_color = ?, enabled = 1 WHERE guild_id = ?",(color_int ,ctx .guild .id ))
                else :
                    await db .execute ("INSERT INTO leveling_settings (guild_id, enabled, embed_color) VALUES (?, 1, ?)",(ctx .guild .id ,color_int ))

                await db .commit ()

            description =f"All level-up message embeds will now use **{color}** as their side-line color."
            if was_disabled :
                description +="\n\n⚡ Leveling was **disabled** — it has been enabled automatically."
    ...
    async def thumbnail (self ,ctx ,setting :str ):
        ...
        try :
            ...
            async with aiosqlite .connect (self .db_path )as db :
                ...
                if exists :
                    await db .execute ("UPDATE leveling_settings SET thumbnail_enabled = ?, enabled = 1 WHERE guild_id = ?",(1 if enabled else 0 ,ctx .guild .id ))
                else :
                    await db .execute ("INSERT INTO leveling_settings (guild_id, enabled, thumbnail_enabled) VALUES (?, 1, ?)",(ctx .guild .id ,1 if enabled else 0 ))
    ...
    async def cooldown (self ,ctx ,seconds :int ):
        ...
        try :
            ...
            async with aiosqlite .connect (self .db_path )as db :
                ...
                if exists :
                    await db .execute ("UPDATE leveling_settings SET cooldown_seconds = ?, enabled = 1 WHERE guild_id = ?",(seconds ,ctx .guild .id ))
                else :
                    await db .execute ("INSERT INTO leveling_settings (guild_id, enabled, cooldown_seconds) VALUES (?, 1, ?)",(ctx .guild .id ,seconds ))
    ...
    async def channel (self ,ctx ,channel :discord .TextChannel ):
        try :
            async with aiosqlite .connect (self .db_path )as db :
                ...
                if result :
                    await db .execute ("UPDATE leveling_settings SET channel_id = ?, enabled = 1 WHERE guild_id = ?",(channel .id ,ctx .guild .id ))
                else :
                    await db .execute ("INSERT INTO leveling_settings (guild_id, enabled, channel_id) VALUES (?, 1, ?)",(ctx .guild .id ,channel .id ))
    ...
    @level .command (name ="background",description ="Set a custom rank card background image",aliases =['bg','image','setbackground'])
    @commands .has_permissions (administrator =True )
    async def background (self ,ctx ,url :Optional [str ]=None ):
        """Set a custom background image for rank cards (URL or attachment)"""
        try :
            if ctx .message .attachments :
                attachment =ctx .message .attachments [0 ]
                if not (attachment .content_type or "").startswith ("image/"):
                    await ctx .send (f"{CROSS} That attachment isn't an image.")
                    return 
                url =attachment .url 

            if not url :
                await ctx .send (
                f"{ZWARNING} Provide an image URL or attach an image.\n"
                f"Usage: `{ctx.prefix}level background <image url>` • `{ctx.prefix}level clearbackground` to reset."
                )
                return 

            url =url .strip ('<>')
            if not re .match (r"^https?://",url ):
                await ctx .send (f"{CROSS} That doesn't look like a valid image URL.")
                return 

            if not PIL_AVAILABLE :
                await ctx .send (f"{CROSS} Image rendering isn't available on this bot instance.")
                return 
            ...
            try :
                ...
                logger .error (f"Invalid rank background: {e}")
                await ctx .send (f"{CROSS} I couldn't load that image. Use a direct image link (png/jpg/webp).")
                return 

            await self .set_rank_background (ctx .guild .id ,url )

            embed =discord .Embed (
            title =f"{TICK} Rank Card Background Set",
            description ="All rank cards and level previews in this server now use this background.\n"
            f"Use `{ctx.prefix}level clearbackground` to go back to the built-in designs.",
            color =0x00BFFF ,
            timestamp =datetime .now (timezone .utc )
            )
            ...
            logger .error (f"Error setting rank background: {e}")
            await ctx .send (f"{CROSS} Something went wrong while saving the background.")

    @level .command (name ="clearbackground",description ="Remove the custom rank card background",aliases =['clearbg','clearimage'])
    @commands .has_permissions (administrator =True )
    async def clearbackground (self ,ctx ):
        """Remove the custom rank card background"""
        try :
            settings =await self .get_guild_settings (ctx .guild .id )
            if not settings .get ('rank_background'):
                await ctx .send (f"{ZWARNING} This server has no custom rank card background set.")
                return 

            await self .set_rank_background (ctx .guild .id ,None )

            embed =discord .Embed (
            title =f"{TICK} Rank Card Background Removed",
            description ="Rank cards will use the built-in designs again.",
            color =0x00BFFF ,
            timestamp =datetime .now (timezone .utc )
            )
            await ctx .send (embed =embed )
            ...
            logger .error (f"Error clearing rank background: {e}")
            await ctx .send (f"{CROSS} Something went wrong while clearing the background.")

    @level .command (name ="preview",description ="Preview the level-up message and rank card")
    async def preview (self ,ctx ,member :Optional [discord .Member ]=None ):
        """Preview how level-ups and rank cards look, with the user's profile picture"""
        member =member or ctx .author 
        try :
            settings =await self .get_guild_settings (ctx .guild .id )
            xp ,level ,messages =await self .get_user_data (ctx .guild .id ,member .id )
            preview_level =max (level ,1 )+1 

            level_message =settings .get ('level_message','Congratulations {user}! You have reached level {level}!')
            level_message =level_message .replace ('{user}',member .mention )
            level_message =level_message .replace ('{level}',str (preview_level ))
            ...
            embed .add_field (name ="Background",value ="Custom image"if settings .get ('rank_background')else "Built-in designs",inline =True )
            embed .add_field (name ="Channel",value =f"<#{settings['channel_id']}>"if settings .get ('channel_id')else "Current channel",inline =True )
            embed .set_footer (text =f"Level {preview_level} • {ctx.guild.name}",icon_url =member .display_avatar .url )

            rank_card =await self .create_rank_card (member ,ctx .guild .id )
            if rank_card and not isinstance (rank_card ,str ):
                file =discord .File (fp =rank_card ,filename ="level_preview.png")
                embed .set_image (url ="attachment://level_preview.png")
                await ctx .send (embed =embed ,file =file )
                try :
                    rank_card .close ()
                except :
            ...
            logger .error (f"Error in level preview command: {e}")
            await ctx .send (f"{CROSS} Something went wrong while building the preview.")
